...
This script returns the following files:
    ...
    * A .png file of plots for TAD (coverage) and ANIr by genome, and
      contig along with a plot for contig length verse TAD.
...
def plot_ANIr_TAD(d, tad, thd, outfile):
    ...
    # Plot labels
    ax5.set_xlabel('Genome Position (bps)', fontsize=14, fontweight='bold')
    ...
    ax6.set_xlabel('Length in Base Pairs', fontsize=14, fontweight='bold')
    ...
    ax5.set_ylabel('ANIr', fontsize=14, fontweight='bold')
    ...
    ax5.set_xlim(left=-5, right=d['ani_cxs']+5)
    ax6.set_xlim(left=-5, right=max(d['aniclens'])+5)

    ax5.set_ylim(bottom=thd-0.5, top=100.5)
